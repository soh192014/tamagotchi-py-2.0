...
class CyberPetGame:
    ...
    def internet_leaderboard(self):
        ...
        try:
            ...
            res = requests.post("https://cyber-pet.taco.center/api/rank", json=payload, timeout=4)
            if res.status_code == 200:
                rank_list = res.json().get("data", [])
                ...
                for idx, player in enumerate(rank_list[:5], 1):
                    ...
                    if p_name == self.name:
                        ...
                        print((GOLD_COLOR + f"  [Rank 1] 👑 {p_name:<16}") if idx == 1 else (Fore.WHITE + f"  [Rank {idx}]    {p_name:<16}") + f" | 核心财富: {p_coins:,} c | 智能等级: Lv.{p_lv}")
            else: raise Exception(f"服务器错误: {res.status_code}")
        except Exception as e:
            print(Fore.RED + f"❌ 链路连接超时。本地天梯降级显示：{e}")
