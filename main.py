...
class CyberPetGame:
    ...
    def start_age_timer(self):
        """启动一个后台守护线程，专门用来给宠物增加年龄"""
        timer_thread = threading.Thread(target=self._age_tracker, daemon=True)
        timer_thread.start()

    def _age_tracker(self):
        """后台年龄轮询核心"""
        while True:
            now = int(time.time())
            # 20分钟 = 20 * 60 = 1200秒
            time_passed = now - self.last_age_update_time
            
            if time_passed >= 1200:
                # 计算过去的时间里应该增加几岁（防止玩家离线/挂机很久后上线一并结算）
                ages_to_add = time_passed // 1200
                self.age += ages_to_add
                # 更新最后一次增加年龄的时间戳
                self.last_age_update_time += ages_to_add * 1200
                # 触发自动存档，防止年龄丢失
                self.save_game()
            
            # 每隔 10 秒在后台检查一次时间戳，极低功耗，绝不卡顿主界面
            time.sleep(10)
    ...
    def save_game(self):
        data = {
            ...
            "last_sign_date": self.last_sign_date, "age": self.age, "last_age_update_time": self.last_age_update_time
        }
    ...
    def init_new_player(self):
        ...
        type_a = inquirer.prompt([inquirer.List("type", message="🧬 请选择血统种类", choices=["乌龟", "猫", "仓束", "狗"])])
        chosen_chinese = type_a["type"] if type_a else "乌龟"
        
        # 🗺️ 跨维映射：转换为底层外部动画库兼容的英文标识符
        type_mapping = {
            "乌龟": "turtle",
            "猫": "cat",
            "仓鼠": "hamster",
        ...
        self.pet_type = type_mapping.get(chosen_chinese, "turtle")
        self.name = p_name
        
        # 最终赋值：既能保底，又能确保后端动画识别不出错
        p_type = type_mapping.get(chosen_chinese, "turtle")
    ...
    def internet_leaderboard(self):
        ...
        # ======= 🛠️ 自动化环境感知：若无 .env 则无感自动生成 =======
        if not os.path.exists(env_file):
            print(Fore.LIGHTBLUE_EX + "⚙️  检测到未配置环境文件，正在为您自动生成标准 .env 轨道矩阵...")
            try:
                with open(env_file, "w", encoding="utf-8") as f:
                    f.write("# =========================================================\n")
                    f.write("# 🪐 赛博朋克电子宠物 - 全局中央环境配置文件\n")
                    f.write("# =========================================================\n\n")
